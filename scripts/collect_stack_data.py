...
MODEL_DIR     = os.path.join(PROJECT_DIR, "models", "mycobot_280")
URDF_FILE     = "mycobot_280_m5_with_camera_flange.urdf"  # URDF including the gripper
EE_LINK_NAME  = "joint6output_to_joint6"                 # end-effector link name
...
def randomize_dynamics(robot_id, mapping):
    for name, idx in mapping.items():
        if p.getJointInfo(robot_id, idx)[2] == p.JOINT_REVOLUTE:
            ...
            p.changeDynamics(robot_id, idx, lateralFriction=base_f + np.random.uniform(-FRIC_VAR, +FRIC_VAR))
...
def pick_and_place(robot_id, ee, mapping, cube, drop_xy):
    ...
    for _ in range(50):
        ...
        for i,j in enumerate(ik_joints):
            p.setJointMotorControl2(robot_id, j, p.POSITION_CONTROL, sol[i], force=200)
        p.stepSimulation()
    ...
    for g in GRIP_JOINTS:
        p.setJointMotorControl2(robot_id, mapping[g], p.POSITION_CONTROL, 0.0, force=50)
    for _ in range(20):
        p.stepSimulation()
    ...
    # 4) Lift & record
    for _ in range(50):
        up = [base[0], base[1], base[2]+0.25]
        sol = p.calculateInverseKinematics(robot_id, ee, up, ee_ori)
        for i,j in enumerate(ik_joints):
            p.setJointMotorControl2(robot_id, j, p.POSITION_CONTROL, sol[i], force=200)
        p.stepSimulation()
        prev_q = record_step(robot_id, ee, mapping, cube, ik_joints, prev_q, traj, sol)

    # 5) Move to drop & record
    drop_above = [drop_xy[0], drop_xy[1], base[2]+0.25]
    for _ in range(50):
        sol = p.calculateInverseKinematics(robot_id, ee, drop_above, ee_ori)
        for i,j in enumerate(ik_joints):
            p.setJointMotorControl2(robot_id, j, p.POSITION_CONTROL, sol[i], force=200)
        p.stepSimulation()
        prev_q = record_step(robot_id, ee, mapping, cube, ik_joints, prev_q, traj, sol)

    # 6) Lower & release (record)
    drop_low = [drop_xy[0], drop_xy[1], base[2]+0.02]
    for _ in range(30):
        sol = p.calculateInverseKinematics(robot_id, ee, drop_low, ee_ori)
        for i,j in enumerate(ik_joints):
            p.setJointMotorControl2(robot_id, j, p.POSITION_CONTROL, sol[i], force=200)
        p.stepSimulation()
        prev_q = record_step(robot_id, ee, mapping, cube, ik_joints, prev_q, traj, sol)
    for g in GRIP_JOINTS:
        p.setJointMotorControl2(robot_id, mapping[g], p.POSITION_CONTROL, 0.04, force=50)
    for _ in range(20):
        p.stepSimulation()
        prev_q = record_step(robot_id, ee, mapping, cube, ik_joints, prev_q, traj, prev_q)
...
def main(episodes, headless):
    ...
    # allow finding your meshes
    p.setAdditionalSearchPath(MODEL_DIR)

    # load robot with gripper
    robot = p.loadURDF(os.path.join(MODEL_DIR, URDF_FILE), [0,0,0], useFixedBase=True)
